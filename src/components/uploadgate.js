...
export default class UploadGate extends Component {
    ...
    onSubmit(e) {
        ...
        axios.patch('https://iitp-mtech-portal-backend.herokuapp.com/backend/applicant/gateScoreCardUpload/'+this.props.match.params.id , formData, {
			...
			window.location.href="https://iitp-mtechportal.netlify.app//mtechstuprofile/"+this.props.match.params.id;
			})
    ...
    render() {
        return (
            <div className="container">
              <div className="name">
            <button className="mtech_btn" onClick ={()=>{window.location.href = "https://iitp-mtechportal.netlify.app//mtechstuprofile/"+this.props.match.params.id}} style={{background:'Salmon',color:"black"}}>Go to Home</button>
            </div>
